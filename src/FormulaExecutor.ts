...
export async function formulaExecutor(
  this: Entity,
  {
    componentName,
    propertyName,
  ...
  code: string = "",
  type: "execution" | "preparation" = "execution",
  ...
  try {
    ...
    const ME = async (
      ...
      try {
        return me.getValue(cmpName, probName);
      } catch (e) {
        console.log("Formula => ME", (e as Error).message);
        return null;
      }
    };
    ...
    const S_ME = (
      ...
      try {
        me.setValue(cmpName, probName, value, true);
        return;
      } catch (e) {
        console.log("Formula => S_ME", (e as Error).message);
      }
    };
    ...
    const FATHER = async (
      ...
      try {
        return father?.getValue(cmpName, probName) || null;
      } catch (e) {
        console.log("Formula => FATHER", (e as Error).message);
        return null;
      }
    };
    ...
    const S_FATHER = (
      ...
      try {
        ...
        console.log("Formula => S_FATHER", (e as Error).message);
      }
    };
    // Получение значения комопнента высшей сущности
    const GRAND_FATHER = async (
      cmpName: string,
      probName: string
    ): Promise<PropertyValue | null> => {
      try {
        return grand_father?.getValue(cmpName, probName) || null;
      } catch (e) {
        console.log("Formula => GRAND_FATHER", (e as Error).message);
        return null;
      }
    };
    // присвоение значения комопненту высшей сущности.
    const S_GRAND_FATHER = (
      cmpName: string,
      probName: string,
      value: PropertyValue
    ): void => {
      try {
        if (!grand_father)
          throw new Error("Родительский объект не существует.");
        me.setValue(cmpName, probName, value, true);
      } catch (e) {
        console.log("Formula => S_GRAND_FATHER", (e as Error).message);
      }
    };
    ...
    const CHILDS = async (
      ...
      try {
        const index = childs.findIndex(
          (b) =>
            b.name.toUpperCase() === entityName.toUpperCase() &&
    ...
    for (const cmp of me.getComponents()) {
      const KEY =
        `me-${cmp.componentName}-${cmp.propertyName}`.toLocaleLowerCase(); // +
      ...
      const GETTER_NAME = IS_CURRENT_PROPERTY
        ? `THIS`
        : `(await ME("${COMPONENT_NAME}", "${PROPERTY_NAME}"))`;
    ...
    for (const cmp of father?.getComponents() || []) {
      const KEY =
        `father-${cmp.componentName}-${cmp.propertyName}`.toLocaleLowerCase(); // +
      const IS_CURRENT_PROPERTY = key === cmp.key; // +
      const ENTITY_NAME = me.name; // Название сущности.  // +
      const ENTITY_NOTE = me.note; // note :) // +
      const COMPONENT_NAME = cmp.componentName; // Название компонента. // +
    ...
    for (const cmp of grand_father?.getComponents() || []) {
      const KEY =
        `grand_father-${cmp.componentName}-${cmp.propertyName}`.toLocaleLowerCase(); // +
      const IS_CURRENT_PROPERTY = key === cmp.key; // +
      const ENTITY_NAME = me.name; // Название сущности.  // +
      const ENTITY_NOTE = me.note; // note :) // +
      const COMPONENT_NAME = cmp.componentName; // Название компонента. // +
    ...
    const groupSet = [
      ...new Set(executorArr.map((e) => e.ENTITY_NAME + "~" + e.ENTITY_NOTE)),
    ]
      .map((g) => g.split("~"))
      .map((g) => ({ name: g[0], note: g[1] || "" }));
    ...
    for (const group of groupSet) {
      const componentNames = [
        ...new Set(
          executorArr
            .filter(
              (e) =>
                e.ENTITY_NAME === group.name && e.ENTITY_NOTE === group.note
            )
      ...
      for (const componentName of componentNames) {
        ...
        buttons.push(
          ...executorArr
            .filter(
              (e) =>
                e.ENTITY_NAME === group.name &&
                e.ENTITY_NOTE === group.note &&
                e.COMPONENT_NAME === componentName
            )
            .map((e) => ({
              name: e.PROPERTY_DESC,
              value: ` = ${e.GETTER_NAME}; // ${e.PROPERTY_TYPE}`,
            }))
        );
      ...
      clientButtons.push({
        group: `${group.name}${
          !group.note || group.note === "" ? "" : " (" + group.note + ")"
        }`,
        components,
      });
    }
      ...
      components: [
        {
          ...
          buttons: [
            ...
            {
              name: "Родительская сущность",
              value: father
                ? 'FATHER("Компонент", "Свойство") '
                : '/*FATHER("Компонент", "Свойство") - undefined :p */',
            },
            {
              name: "Высшая сущность",
              value: grand_father
                ? 'GRAND_FATHER("Компонент", "Свойство") '
                : '/*GRAND_FATHER("Компонент", "Свойство") - undefined :p */',
            },
            {
              name: "Братья",
              value:
                'BROTHERS("Название сущности", "Заметка", "Компонент", "Свойство") ',
            },
            {
              name: "Дочерние сущности",
              value:
                'CHILDS("Название сущности", "Заметка", "Компонент", "Свойство") ',
            },
          ...
          setters: [
            {
              name: "Текущая сущность",
              value: 'S_ME("Компонент", "Свойство", "Значение") ',
            },
            {
              name: "Родительская сущность",
              value: 'S_FATHER("Компонент", "Свойство", "Значение") ',
            },
            {
              name: "Высшая сущность",
              value: 'S_GRAND_FATHER("Компонент", "Свойство", "Значение") ',
            },
            {
              name: "Братья",
              value:
                'S_BROTHERS("Название сущности", "Заметка", "Компонент", "Свойство", "Значение") ',
            },
            {
              name: "Дочерние сущности",
              value:
                'S_CHILDS("Название сущности", "Заметка", "Компонент", "Свойство", "Значение") ',
            },
            { name: "Результат", value: "RESULT = " },
          ],
        },
