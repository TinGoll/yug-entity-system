...
export default class Entity {
  ...
  getComponent(componentName: string): Component | null {
    const componentsApi = this.getComponents().filter(
      (cmp) => cmp.componentName === componentName
    );
  ...
  async addChildToKey(
    ...
    const newChildShell = await this._engine.cloneEntityShell(
      key,
      this.key,
      response,
      reject
  ...
  existComponent(componentName: string, propertyName: string): boolean {
    return Boolean(
      this._shell.options.components.find(
        (c) =>
  ...
  async getChangedEntities(): Promise<Entity[]> {
    ...
    for (const iterator of entityShells) {
      if (
        iterator.options?.indicators.is_changeable ||
        iterator.options?.indicators.is_changeable_component
      ) {
  ...
  getNotRecordedComponents(): ApiComponent[] {
    return this._shell.options.components.filter(
      (cmp) => cmp.indicators.is_unwritten_in_storage
    );
  }

  getNotUpdatedComponents(): ApiComponent[] {
    return this._shell.options.components.filter(
      (cmp) => cmp.indicators.is_changeable
    );
  }

  getNotNotificated(): ApiComponent[] {
    return this._shell.options.components.filter(
      (cmp) => cmp.indicators.is_not_sent_notification
    );
  }
  ...
  async getValue(
    ...
    try {
      const cmp = this._shell.options.components?.find(
        (c) =>
          c.componentName === componentName && c.propertyName === propertyName
      );
      if (!cmp)
        throw new Error(
          `getValue: <${this.name} (${this.category})> Свойство ${<string>(
            propertyName
          )} компонента ${componentName} не найдено.`
        );
  ...
  async getValueToKey(key: string): Promise<PropertyValue | null> {
    try {
      ...
      if (!cmp)
        throw new Error(
          `getValueToKey: <${this.name} (${this.category})> Свойство ключу <${key}> не найдено.`
        );
      return this.get_property_value(cmp);
    } catch (e) {
      console.log(e);
  ...
  setValueToKey(
    ...
    if (!cmp)
      throw new Error(
        `getValueToKey: <${this.name} (${this.category})> Свойство по ключу <${key}> не найдено.`
      );
  ...
  private set_property(
    ...
    try {
      ...
      if (isReadonly && manualChange)
        throw new Error(
          "Изменение свойства запрещено, так как установлен 'readonly' атрибут."
        );
      switch (type) {
        case "string":
   ...
   */
  private async get_property_value(
    cmp: ApiComponent
  ): Promise<PropertyValue | null> {
    try {
  ...
  private convert_value_by_type(type: PropertyType, value: PropertyValue) {
    switch (type) {
      case "string":
        return String(value); // Преобразование в строку
      case "boolean":
        return Boolean(value); // Преобразование в boolean
      case "number":
        return Number(value); // Преобразование в число
      case "date":
        return new Date(<string>value); // Преобразование в дату
      default:
        return value; // возврат значения в том же виде.
    }
  }
  ...
  private attribute_exists(
    ...
    try {
      ...
      return !!attributeArr.find(
        (a) => a.toUpperCase() === findAtt.toUpperCase()
      );
    } catch (e) {
  ...
  getId(): number {
    return this._shell.options.id || 0;
  }
  getComponents(): ApiComponent[] {
    return this._shell.options.components || [];
  }
  /** Список (объект) индикаторов сущности */
  getIndicators(): EntityIndicators {
    return this._shell.options.indicators;
  }
  /** Индекс сущности */
  getIndex(): number {
    return this._shell.options.index || 0;
  }
  /** Ключ сущности */
  getKey(): string {
    return this._shell.options.key || "";
  }
  /** Родительский ключ сущности */
  getParentKey(): string | null {
    return this._shell.options.parentKey || null;
  }
  /** Ключ шаблона сущности */
  getSampleKey(): string | null {
    return this._shell.options.sampleKey || null;
  }
  /** Описание сущности */
  getNote(): string {
    return this._shell.options.note || "";
  }
  /** Тип сущности */
  getType(): string {
    return this._shell.options.type || "";
  }
  /** Сатегория сущности */
  getCategory(): string {
    return this._shell.options.category || "";
  }
  /** Имя сущности */
  getName(): string {
    return this._shell.options.name;
  }
  ...
  /** Ключ шаблона сущности */
  get sampleKey(): string | null {
    return this.getSampleKey();
  }
  /** Получить список компонентов сущности */
  ...
  /** Список (объект) индикаторов сущности */
  get indicators(): EntityIndicators {
